...
// --- LUCIDE ICONS (Fully Populated) ---
import {
  Settings, X, ChevronDown, CheckCircle, Github, Code2, AlertTriangle, 
  History, BookOpen, Copy, Download, FileCode, SearchCode, Play, Menu, 
  Plus, ShieldCheck, Terminal, Scale, Sword, Key, Eye, EyeOff, Cpu, Database
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  // --- LOCAL API KEY VAULT (DECENTRALIZED STORAGE) ---
  ...
  const [alertConfig, setAlertConfig] = useState<AlertConfig>({ show: false, message: '' });
  ...
  // --- UTILS ---
  const showAlert = (message: string, action?: () => void, actionText?: string) =>
    setAlertConfig({ show: true, message, action, actionText });
  const hideAlert = () => setAlertConfig({ show: false, message: '' });

  // ============================================================================
  // REACT EFFECTS (LIFECYCLES)
  // ============================================================================
  
  // 1. Handle Screen Resizing for Mobile/Desktop behaviors
  useEffect(() => {
  ...
  // 2. Prevent body scrolling when Mobile menu is open
  useEffect(() => {
    document.body.style.overflow = isMobileSidebarOpen ? 'hidden' : 'unset';
    return () => { document.body.style.overflow = 'unset'; };
  }, [isMobileSidebarOpen]);

  // 3. Reset PDF exporting state if print dialogue is cancelled
  useEffect(() => {
  ...
  // 4. Auto-scroll the live telemetry terminal to the bottom
  useEffect(() => {
  ...
  // 5. Load Scan History from local storage on mount
  useEffect(() => {
  ...
  // Save API keys directly to browser storage
  ...
  const saveToHistory = (result: AuditResult, method: string, title: string) => {
    ...
    const updatedHistory = [newItem, ...historyItems].slice(0, 50); // Keep last 50 scans
  ...
  // The Primary Audit Execution Engine
  const handleRunAudit = async () => {
    if (step === 'analyzing') return;

    // Security Check: At least one LLM key must exist
    const hasLlmKey =
      apiKeys.gemini ||
      apiKeys.deepseek ||
      apiKeys.minimax ||
    ...
    if (!hasLlmKey) {
      setStep('settings');
      return showAlert(
        'Configuration Required: Please configure at least one API key in Settings before running a scan.'
      );
    }
    ...
    if (auditMethod === 'code') {
      ...
      if (!contractAddress.trim())
        return showAlert('Please enter a valid Ethereum Contract Address.');
      if (!apiKeys.etherscan)
        return showAlert('An Etherscan API Key is required to fetch contracts by address. Please add it in Settings.');
      payload.code = contractAddress;
      
      // SAFE SUBSTRING FIX: Ensures string exists and is long enough before slicing
      auditTitle = contractAddress && contractAddress.length >= 10 
        ? `Contract: ${contractAddress.substring(0, 6)}...${contractAddress.substring(contractAddress.length - 4)}` 
        : "Contract Audit";
        
    } else if (auditMethod === 'github') {
      if (!githubUrl.trim())
        return showAlert('Please explicitly type the repository (e.g., blockvia/sential).');
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/audit/stream`,
        {
          method: 'POST',
          headers: headers,
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              if (eventData.type === 'log') {
                ...
                // Handle backend failure securely
              ...
              // Ignore split JSON chunks during rapid streaming
            }
          }
        }
      }
      ...
      showAlert(
        error.message || 'Audit Failed. Please verify your API Keys and local server connection.'
      );
      setStep('scanner');
    }
  };
  ...
  const renderMainContent = () => {
      ...
      return (
        <div className="flex-1 flex flex-col items-center justify-start p-4 pt-8 md:p-12 md:pt-16 animate-in fade-in duration-500 max-w-[1000px] mx-auto w-full min-w-0">
          ...
          <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-end mb-[32px] md:mb-[40px] border-b border-[var(--border-subtle)] pb-4 md:pb-6 gap-2 md:gap-0">
            <div>
              <h2 className="text-[24px] md:text-[32px] font-medium tracking-tight text-[var(--text-1)] mb-1 md:mb-2">
                New Consensus Scan
              </h2>
              <p className="text-[13px] md:text-[15px] text-[var(--text-2)] font-light">
                Select ingestion method to initialize semantic RAG parsing.
              </p>
            </div>
          </div>
          ...
          <div className="w-full bg-[var(--bg-surface-1)] border border-[var(--border-subtle)] rounded-[16px] md:rounded-[24px] p-[20px] md:p-[32px] shadow-sm flex flex-col gap-[24px] md:gap-[32px] min-w-0">
            ...
            {/* Step 1: Ingestion Method Selectors */}
            <div className="flex flex-col gap-3 w-full">
              <span className="text-[11px] md:text-[12px] font-bold tracking-[0.08em] text-[var(--text-3)] uppercase">
                1. Ingestion Method
              </span>
                ...
                {[
                  ...
                  <div
                    ...
                    className={`relative text-[12px] md:text-[14px] font-medium px-[12px] md:px-[20px] py-[10px] rounded-[8px] cursor-pointer transition-all duration-300 flex items-center justify-center gap-2 flex-1 sm:flex-none whitespace-nowrap
                      ${auditMethod === method.id
                        ? 'bg-[var(--bg-surface-4)] text-[var(--text-1)] shadow-md'
                        : 'text-[var(--text-3)] hover:text-[var(--text-2)] hover:bg-[var(--bg-surface-3)]'
                      }`}
                  >
                    {method.label}
                  </div>
            ...
            {/* Step 2: Target Configuration Input Area */}
                ...
                {auditMethod === 'code' && (
                  <div className="bg-[var(--bg-surface-2)] border border-[var(--border-default)] rounded-[12px] md:rounded-[16px] overflow-hidden flex flex-col focus-within:border-[var(--border-strong)] transition-colors shadow-inner w-full min-w-0">
                    <div className="bg-[var(--bg-surface-3)] border-b border-[var(--border-subtle)] px-[16px] md:px-[20px] py-[12px] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <span className="text-[11px] md:text-[12px] text-[var(--text-3)] font-mono flex items-center gap-2 shrink-0">
                        <FileCode size={14} /> contract.sol
                      </span>
                      <div className="flex flex-wrap items-center gap-2 md:gap-4 shrink-0 w-full sm:w-auto">
                        ...
                        <button
                          ...
                          className="text-[11px] md:text-[12px] font-medium text-[var(--text-2)] hover:text-[var(--text-1)] transition-colors bg-[var(--bg-surface-4)] sm:bg-transparent px-3 py-1.5 rounded-md sm:px-0 sm:py-0 flex-1 sm:flex-none text-center border border-[var(--border-default)] sm:border-none"
                        >
                          Upload File
                        </button>
                        <input
                    ...
                    <textarea
                      ...
                      className="w-full h-[300px] md:h-[400px] bg-transparent text-[var(--text-1)] font-mono text-[13px] md:text-[14px] p-[16px] md:p-[24px] resize-none focus:outline-none leading-loose custom-scrollbar min-w-0"
                      spellCheck={false}
                    />
                  </div>
                )}
                ...
                {auditMethod === 'address' && (
                  <div className="bg-[var(--bg-surface-2)] border border-[var(--border-default)] rounded-[12px] md:rounded-[16px] p-[24px] md:p-[32px] flex flex-col items-center shadow-inner w-full min-h-[250px] justify-center">
                    ...
                    <input
                      ...
                      placeholder="0x..."
                      className="w-full max-w-[500px] bg-[var(--bg-base)] border border-[var(--border-strong)] rounded-[10px] md:rounded-[12px] px-[16px] md:px-[24px] py-[14px] md:py-[16px] text-[var(--text-1)] font-mono focus:border-[var(--accent)] focus:outline-none transition-colors text-[13px] md:text-[14px] shadow-inner"
                    />
                  </div>
                )}
                ...
                {auditMethod === 'github' && (
                  <div className="bg-[var(--bg-surface-2)] border border-[var(--border-default)] rounded-[12px] md:rounded-[16px] p-[24px] md:p-[32px] flex flex-col items-center text-center shadow-inner w-full min-h-[250px] justify-center">
                    ...
                    <div className="w-full flex flex-col items-center">
                      <p className="text-[var(--text-2)] mb-[20px] md:mb-[24px] max-w-md text-[13px] md:text-[15px]">
                        Enter the public GitHub repository you want to scan (e.g.{' '}
                      ...
                      <input
                        ...
                        placeholder="blockvia/sential"
                        className="w-full max-w-[500px] bg-[var(--bg-base)] border border-[var(--border-strong)] rounded-[10px] md:rounded-[12px] px-[16px] md:px-[24px] py-[14px] md:py-[16px] text-[var(--text-1)] font-mono focus:border-[var(--accent)] focus:outline-none transition-colors text-[13px] md:text-[14px] shadow-inner mb-4"
                      />
                      ...
                      <div className="w-full max-w-[500px] flex items-center justify-between mt-2 bg-[var(--bg-surface-1)] border border-[var(--border-default)] p-4 rounded-xl text-left">
                        <div className="flex flex-col">
                          <span className="text-[14px] font-bold text-[var(--text-1)] flex items-center gap-2">
                            <ShieldCheck size={16} className="text-[#B066FF]" /> Deep Supply Chain Scan
                          </span>
                          <span className="text-[12px] text-[var(--text-3)] mt-1">
                            Include node_modules &amp; lib/ to detect malicious dependency injections.
                          </span>
                        </div>
                        <div
                          onClick={() => setIsDeepScanEnabled(!isDeepScanEnabled)}
                          className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-colors duration-300 ease-in-out shrink-0 ${
                            isDeepScanEnabled
                              ? 'bg-[#34C759]'
                        ...
                        >
                          <div
                            className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-300 ${
                              isDeepScanEnabled ? 'translate-x-6' : 'translate-x-0'
                            }`}
                          />
                        </div>
                      </div>
            ...
            <div className="pt-[24px] border-t border-[var(--border-subtle)] w-full shrink-0 flex flex-col gap-4">
              <button
                ...
                className={`w-full h-[50px] md:h-[56px] rounded-[10px] md:rounded-[12px] font-semibold text-[15px] md:text-[16px] text-[var(--bg-base)] bg-[var(--accent)] transition-all duration-300 flex items-center justify-center gap-3 overflow-hidden group relative shrink-0 shadow-lg
                  ${
                    contractCode.length > 0 || contractAddress.length > 0 || githubUrl.length > 0
                      ? 'hover:scale-[1.01] active:scale-[0.98]'
                      : 'bg-[var(--bg-surface-3)] text-[var(--text-3)] cursor-not-allowed border border-[var(--border-strong)] opacity-50'
                  }`}
              >
                <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-[rgba(255,255,255,0.2)] to-transparent -translate-x-full group-hover:animate-shimmer" />
                <span className="relative z-10 flex items-center gap-2">
                  <Play size={16} fill="currentColor" /> Initialize Security Invariant Mapping
                </span>
              </button>
            </div>
    ...
    // ---------------------------------------------------------
    // VIEW 2: THE HACKER TERMINAL (LOADING STATE)
    // ---------------------------------------------------------
    if (step === 'analyzing') {
      return (
          ...
          <div className="w-full max-w-[1000px] bg-[#0A0D14] border border-[var(--border-strong)] rounded-2xl shadow-[0_0_50px_rgba(0,194,255,0.05)] overflow-hidden flex flex-col h-[65vh] min-h-[450px]">
            ...
            <div className="bg-[#111622] border-b border-[var(--border-strong)] px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-[#FF3B30] border border-[rgba(255,59,48,0.5)]" />
                <div className="w-3 h-3 rounded-full bg-[#FFB800] border border-[rgba(255,184,0,0.5)]" />
                <div className="w-3 h-3 rounded-full bg-[#34C759] border border-[rgba(52,199,89,0.5)]" />
              </div>
            ...
            <div className="flex-1 p-6 md:p-8 overflow-y-auto font-mono text-[12px] md:text-[14px] leading-relaxed custom-scrollbar flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2 mt-2 text-[#34C759] opacity-80">
                <span className="w-2.5 h-4 bg-[#34C759] animate-pulse" />
              </div>
    ...
    if (step === 'results' && auditResult) {
      ...
      return (
        <div
          ...
          className={`flex-1 flex flex-col items-center justify-start w-full min-w-0 mx-auto animate-in slide-in-from-bottom-10 duration-700 bg-[var(--bg-base)] ${
            isExporting ? 'max-w-none p-6 md:p-12' : 'max-w-[1100px] p-4 pt-8 md:p-12'
          }`}
        >
          {/* Report Header */}
          <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-end mb-[32px] md:mb-[48px] border-b border-[var(--border-subtle)] pb-[24px] md:pb-[32px] gap-4 md:gap-0 shrink-0">
            <div>
              <div className="flex items-center gap-[10px] md:gap-[12px] mb-[8px] md:mb-[16px]">
                <div className="w-2 h-2 rounded-full bg-[var(--low)] shadow-[0_0_8px_rgba(52,199,89,0.8)] animate-pulse shrink-0" />
                <span className="text-[10px] md:text-[11px] font-bold tracking-[0.15em] text-[var(--text-3)] uppercase">
                  Audit Complete
                </span>
              </div>
              <h2 className="font-display text-[32px] md:text-[48px] tracking-[-0.02em] text-[var(--text-1)] leading-none">
                Consensus Report.
              </h2>
            </div>
            {!isExporting && (
              <div className="flex items-center gap-[12px] md:gap-[20px] w-full md:w-auto shrink-0">
                ...
                <button
                  onClick={resetApp}
                  className="flex-1 md:flex-none text-[12px] md:text-[13px] font-semibold uppercase tracking-[0.08em] text-[var(--text-2)] hover:text-[var(--text-1)] transition-colors bg-[var(--bg-surface-2)] border border-[var(--border-default)] px-[16px] md:px-[20px] py-[10px] rounded-[8px] whitespace-nowrap text-center"
                >
                  New Scan
                </button>
              </div>
          ...
          <div
            className={`grid grid-cols-1 md:grid-cols-3 gap-[32px] md:gap-[48px] w-full mb-[40px] md:mb-[64px] bg-[var(--bg-surface-1)] rounded-[20px] md:rounded-[24px] p-[24px] md:p-[40px] shrink-0 ${
              isExporting ? 'border-none' : 'border border-[var(--border-subtle)] shadow-sm'
            }`}
          >
            {/* Score Dial */}
            <div className="flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-[var(--border-subtle)] pb-8 md:pb-0">
              <div className="relative w-[140px] h-[140px] md:w-[180px] md:h-[180px] flex items-center justify-center shrink-0">
                <svg
                  className="absolute inset-0 w-full h-full transform rotate-[130deg]"
                  viewBox="0 0 100 100"
                >
                  <circle
                    cx="50" cy="50" r="45"
                  ...
                  <circle
                    ...
                    strokeWidth="4"
                    strokeDasharray={`${(scorePercentage / 100) * 226} 314`}
                    strokeLinecap="round"
                    className="animate-drawArc drop-shadow-[0_0_8px_currentColor]"
                  />
                </svg>
                <div className="flex flex-col items-center mt-2 md:mt-3">
                  <span className="font-display text-[48px] md:text-[64px] leading-none text-[var(--text-1)] tracking-tight">
                    {auditResult.score}
                  </span>
                  <span className="text-[12px] md:text-[14px] text-[var(--text-3)] font-mono mt-1">/ 10</span>
                </div>
              </div>
              <h3 className="text-[10px] md:text-[11px] font-bold text-[var(--text-3)] mt-[20px] md:mt-[24px] tracking-[0.15em] uppercase text-center">
                Security Score
              </h3>
            </div>
            ...
            <div className="md:col-span-2 flex flex-col justify-center min-w-0">
              <span className="text-[10px] md:text-[11px] font-bold text-[var(--text-3)] mb-[12px] md:mb-[16px] tracking-[0.15em] uppercase text-center md:text-left">
                Executive Summary
              </span>
          ...
          {auditResult.invariants && auditResult.invariants.length > 0 && (
            <div className="w-full flex flex-col gap-[16px] mb-[40px] md:mb-[64px] min-w-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-[12px] md:gap-[16px]">
                {auditResult.invariants.map((inv: any, idx: number) => (
                  <div
                    key={idx}
                    className="bg-[var(--bg-surface-2)] border border-[var(--border-default)] p-[16px] md:p-[20px] rounded-[12px] shadow-sm flex items-start gap-4"
                  >
    ...
    // ---------------------------------------------------------
    // VIEW 4: SETTINGS (LOCAL API KEY VAULT)
    // ---------------------------------------------------------
    if (step === 'settings') {
      const renderKeyInput = (
        ...
        <div className="flex flex-col gap-2 w-full">
          <label className="text-[12px] font-bold text-[var(--text-3)] uppercase tracking-widest">
            {label}
          </label>
      ...
      return (
        <div className="flex-1 flex flex-col items-center justify-start p-6 pt-12 md:pt-24 max-w-[800px] mx-auto w-full animate-in fade-in">
          <div className="w-full mb-8 md:mb-12 border-b border-[var(--border-subtle)] pb-6 flex justify-between items-end">
            <h2 className="text-[28px] md:text-[32px] font-medium text-[var(--text-1)]">
              Configuration Vault
            </h2>
          ...
          <div className="w-full bg-[var(--bg-surface-1)] border border-[var(--border-subtle)] rounded-[20px] p-6 md:p-8 shadow-sm flex flex-col gap-8 mb-12">
            ...
            <div>
              <h3 className="text-[18px] font-bold text-[var(--text-1)] mb-2 flex items-center gap-2">
                <Cpu className="text-[var(--accent)]" /> AI Providers
              </h3>
              <p className="text-[13px] text-[var(--text-2)] mb-6">
                Keys are stored securely in your browser's local storage and are only transmitted
                directly to your local Docker container during a scan. At least one key is required.
              </p>
            ...
            <div className="border-t border-[var(--border-subtle)] pt-8">
              <h3 className="text-[18px] font-bold text-[var(--text-1)] mb-2 flex items-center gap-2">
                <Database className="text-[var(--low)]" /> Blockchain Integration
              </h3>
              <p className="text-[13px] text-[var(--text-2)] mb-6">
                Required if you intend to fetch smart contract source code directly via verified
                block explorer URLs.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    // ---------------------------------------------------------
    // VIEW 5: SCAN HISTORY (LOCAL STORAGE)
    // ---------------------------------------------------------
    if (step === 'history') {
      return (
        <div className="flex-1 flex flex-col items-center justify-start p-4 pt-8 md:p-12 animate-in fade-in duration-500 w-full min-w-0 max-w-[1200px] mx-auto">
          ...
          <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-end mb-[24px] md:mb-[40px] border-b border-[var(--border-subtle)] pb-[20px] md:pb-[24px] gap-4 sm:gap-0 shrink-0">
            <div>
              <h2 className="text-[28px] md:text-[32px] font-medium tracking-tight text-[var(--text-1)] mb-2">
                Scan History
              </h2>
              <p className="text-[13px] md:text-[15px] text-[var(--text-2)] font-light">
                Recent audits are saved securely in your local browser cache.
              </p>
            </div>
            <div className="flex items-center gap-4">
              <button
                onClick={() => { localStorage.removeItem('sential_history'); setHistoryItems([]); }}
                className="text-[11px] text-[var(--critical)] border border-[var(--critical)] hover:bg-[var(--critical)] hover:text-white px-3 py-1.5 rounded transition-colors"
              >
                Clear History
              </button>
              <span className="text-[var(--text-3)] font-mono text-[12px] md:text-[14px] bg-[var(--bg-surface-1)] px-3 py-1.5 rounded-md border border-[var(--border-subtle)] whitespace-nowrap shadow-sm shrink-0">
              ...
              <table className="w-full text-left border-collapse min-w-[600px]">
                <thead>
                  <tr className="border-b border-[var(--border-strong)] bg-[var(--bg-surface-2)]">
                    {['Date', 'Target', 'Method', 'Score', 'Action'].map((h, i) => (
                      <th
                        key={h}
                        className={`py-[16px] px-[20px] md:px-[24px] font-bold text-[10px] md:text-[11px] tracking-[0.08em] uppercase text-[var(--text-3)] whitespace-nowrap ${i === 4 ? 'text-right' : ''}`}
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="text-[13px] md:text-[14px] text-[var(--text-2)]">
                  {historyItems.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-[64px] text-center text-[var(--text-3)] font-mono text-[13px]">
                    ...
                    historyItems.map(item => (
                      <tr
                        key={item.id}
                        className="border-b border-[var(--border-subtle)] hover:bg-[var(--bg-surface-2)] transition-colors group cursor-pointer"
                        onClick={() => loadPastAudit(item)}
                      >
                        ...
                        <td className="py-[16px] px-[20px] md:px-[24px] text-[var(--text-1)] font-medium max-w-[150px] md:max-w-[200px] truncate">
                          {item.title}
                        </td>
                        ...
                        <td className="py-[16px] px-[20px] md:px-[24px] text-right whitespace-nowrap">
                          <span className="text-[12px] md:text-[13px] font-medium text-[var(--text-3)] group-hover:text-[var(--accent)] transition-colors">
                            View Report →
                          </span>
                        </td>
                      </tr>
        ...
        <div className="flex-1 flex flex-col items-center justify-start p-6 pt-24 max-w-[900px] mx-auto w-full animate-in fade-in">
          ...
          <div className="w-full flex flex-col gap-8 pb-20">
            ...
            <div className="bg-[var(--bg-surface-1)] border border-[var(--border-subtle)] p-8 rounded-[20px] shadow-sm">
              <h3 className="text-[18px] font-bold text-[var(--text-1)] mb-4 flex items-center gap-2">
                <Sword className="text-[#FF3B30]" /> The Adversarial Triad Engine
              </h3>
              <p className="text-[14px] text-[var(--text-2)] leading-relaxed mb-4">
                Sential AI does not rely on a single monolithic LLM. It utilizes a margin-protected
                waterfall architecture to detect vulnerabilities that standard linters miss:
              </p>
              <ul className="list-disc list-inside text-[14px] text-[var(--text-3)] space-y-2 ml-4 font-mono">
                <li>
                  <strong className="text-[var(--text-2)] font-sans">Gatekeeper (Llama 3.3 8B):</strong>{' '}
                  Isolates high-risk logic vectors and strips boilerplate to conserve token context.
                </li>
                <li>
                  <strong className="text-[var(--text-2)] font-sans">Red Team (DeepSeek / Gemini):</strong>{' '}
                  Aggressively synthesizes complex attack vectors and writes weaponized Foundry PoC code.
                </li>
                <li>
                  <strong className="text-[var(--text-2)] font-sans">Judge (Claude 3 Opus / Zhipu):</strong>{' '}
                  Cryptographically verifies the logic and standardizes the severity of the output.
                </li>
              </ul>
            </div>
            ...
            <div className="bg-[var(--bg-surface-1)] border border-[var(--border-subtle)] p-8 rounded-[20px] shadow-sm">
              <h3 className="text-[18px] font-bold text-[var(--text-1)] mb-4 flex items-center gap-2">
                <Terminal className="text-[#34C759]" /> The Guillotine Sandbox
              </h3>
              <p className="text-[14px] text-[var(--text-2)] leading-relaxed">
                AI hallucinations are the biggest bottleneck in automated auditing. Sential eliminates
                false positives by taking the Red Team's generated <code>.t.sol</code> file and injecting
                it into your local Foundry compiler. If the Foundry test reverts, Sential silently
                downgrades the alert. We prove the math, we don't just guess.
              </p>
            </div>
  ...
  return (
    <>
      ...
      <div
        className={`flex w-full min-w-0 bg-[var(--bg-base)] font-ui relative selection:bg-[var(--accent-dim)] selection:text-[var(--text-1)] ${
          isExporting ? 'h-auto min-h-screen block' : 'h-screen overflow-hidden'
        }`}
      >
        ...
        {!isExporting && (
          <div className="md:hidden shrink-0 fixed top-0 left-0 right-0 z-30 h-[60px] bg-[rgba(7,9,15,0.85)] backdrop-blur-xl border-b border-[var(--border-subtle)] flex items-center justify-between px-4 print:hidden w-full">
            <button
              ...
              className="p-2 -ml-2 text-[var(--text-1)] active:scale-95 transition-transform shrink-0"
            >
              <Menu size={24} />
            </button>
            <div className="flex-1 flex justify-center min-w-0">
              <img
                src={activeLogo}
                alt="Sential AI"
                className="h-[20px] w-auto object-contain drop-shadow-[0_0_10px_rgba(0,194,255,0.2)]"
              />
            </div>
            <button
              onClick={() => { resetApp(); window.scrollTo(0, 0); }}
              className="p-2 -mr-2 text-[var(--text-1)] active:scale-95 transition-transform shrink-0"
            >
              <Plus size={24} />
            </button>
          </div>
        ...
        {alertConfig.show && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl animate-in fade-in duration-200 print:hidden w-full h-full">
            <div className="bg-[var(--bg-surface-1)] border border-[var(--border-subtle)] rounded-[20px] md:rounded-[24px] p-8 md:p-10 max-w-sm w-full relative shadow-2xl text-center flex flex-col items-center">
              ...
              <p className="text-[13px] md:text-[14px] text-[var(--text-2)] mb-6 md:mb-8 leading-relaxed w-full break-words">
                {alertConfig.message}
              </p>
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full shrink-0">
                <button
                  onClick={hideAlert}
                  className="flex-1 bg-[var(--bg-surface-2)] border border-[var(--border-default)] text-[var(--text-1)] py-2.5 md:py-3 rounded-[8px] md:rounded-[10px] transition-all font-semibold text-[13px] w-full"
                >
                  Dismiss
                </button>
                {alertConfig.action && (
                  <button
                    onClick={() => { alertConfig.action!(); hideAlert(); }}
                    className="flex-1 bg-[var(--text-1)] text-[var(--bg-base)] py-2.5 md:py-3 rounded-[8px] md:rounded-[10px] transition-all font-semibold text-[13px] w-full"
                  >
        ...
        {/* --- RESPONSIVE SIDEBAR --- */}
        <div
          className={`fixed inset-y-0 left-0 z-[50] bg-[var(--bg-surface-1)] border-r border-[var(--border-subtle)] flex flex-col transition-all duration-300 ease-[var(--ease-spring)] print:hidden shrink-0
        ...
        >
          ...
          <div className="h-[60px] md:h-[72px] flex items-center justify-between px-4 md:px-5 border-b border-[var(--border-subtle)] shrink-0">
            <div
              className="flex items-center cursor-pointer group overflow-hidden w-full min-w-0"
              onClick={() => { resetApp(); if (isMobile) setIsMobileSidebarOpen(false); }}
            >
              {!isDesktopSidebarOpen && !isMobile ? (
                <img
                  src={activeIcon}
                  alt="Sential AI"
                  className="h-[28px] w-[28px] object-contain mx-auto group-hover:scale-110 transition-transform shrink-0"
                />
              ) : (
                <img
                  src={activeLogo}
                  alt="Sential AI"
                  className="h-[22px] md:h-[24px] w-auto object-contain group-hover:scale-105 transition-transform"
                />
              )}
            </div>
            ...
            {/* Mobile Close Toggle */}
            <button
              className="md:hidden p-2 -mr-2 text-[var(--text-3)] hover:text-[var(--text-1)] shrink-0"
              onClick={() => setIsMobileSidebarOpen(false)}
            >
              <X size={24} />
            </button>
          ...
          <div className="flex-1 overflow-y-auto py-[16px] md:py-[24px] flex flex-col gap-[4px] custom-scrollbar min-w-0">
            <SidebarItem
              icon={<Code2 size={18} />}
              label="Scanner"
              active={step === 'scanner' || step === 'analyzing' || step === 'results'}
              onClick={() => { resetApp(); if (isMobile) setIsMobileSidebarOpen(false); }}
              isCollapsed={!isDesktopSidebarOpen && !isMobile}
            />
            <SidebarItem
              icon={<History size={18} />}
              label="History"
              active={step === 'history'}
              onClick={() => { setStep('history'); if (isMobile) setIsMobileSidebarOpen(false); }}
              isCollapsed={!isDesktopSidebarOpen && !isMobile}
            />
            <SidebarItem
              ...
              onClick={() => { setStep('settings'); if (isMobile) setIsMobileSidebarOpen(false); }}
              isCollapsed={!isDesktopSidebarOpen && !isMobile}
            />
            <div className="mt-4 mb-2 border-t border-[var(--border-subtle)] mx-4 shrink-0" />
            <SidebarItem
              icon={<BookOpen size={18} />}
              label="Documentation"
              active={step === 'docs'}
              onClick={() => { setStep('docs'); if (isMobile) setIsMobileSidebarOpen(false); }}
              isCollapsed={!isDesktopSidebarOpen && !isMobile}
            />
          </div>
        ...
        <div
          className={`flex-1 w-full min-w-0 relative flex flex-col z-10 transition-all duration-300 ease-[var(--ease-spring)]
    ...
    <div
      ...
      className={`flex items-center cursor-pointer transition-all duration-200 h-[44px] rounded-[10px] mx-2 mb-1 w-auto min-w-0 shrink-0
        ...
        ${active
          ...
          : 'text-[var(--text-2)] hover:bg-[var(--bg-surface-2)] hover:text-[var(--text-1)]'
        }`}
      title={isCollapsed ? label : undefined}
    >
      <div className={`shrink-0 ${active ? 'text-[var(--accent)]' : 'text-[var(--text-3)]'}`}>
        {icon}
      </div>
      {!isCollapsed && (
        <span className="text-[14px] font-medium whitespace-nowrap truncate w-full">{label}</span>
      )}
    </div>
  );
}

// 2. Expandable Severity Accordion Component
function SeverityAccordion({ type, label, items, forceOpen, showAlert }: any) {
  ...
  return (
    <div
      className={`w-full min-w-0 bg-[var(--bg-surface-1)] border border-[var(--border-subtle)] rounded-[12px] md:rounded-[16px] overflow-hidden shrink-0 ${
        forceOpen ? 'break-inside-avoid shadow-none' : 'shadow-sm'
      }`}
    >
      {/* Accordion Header */}
      <div
        onClick={() => count > 0 && setIsOpen(!isOpen)}
        className={`px-[16px] md:px-[24px] h-[56px] md:h-[64px] flex justify-between items-center transition-colors bg-[var(--bg-surface-2)] border-b border-transparent w-full min-w-0 shrink-0
          ${isOpen ? 'border-[var(--border-subtle)]' : ''}
          ${count > 0 ? 'cursor-pointer hover:bg-[var(--bg-surface-3)]' : 'opacity-50 cursor-not-allowed'}`}
      >
        <div className="flex items-center gap-[12px] md:gap-[16px] min-w-0 shrink-0">
          <div
            className={`w-[8px] h-[8px] md:w-[10px] md:h-[10px] rounded-full shrink-0 ${styles[type].dot} ${
              isOpen ? 'animate-pulse shadow-[0_0_8px_currentColor]' : ''
            }`}
          />
          <span className="font-medium text-[14px] md:text-[15px] text-[var(--text-1)] truncate max-w-[200px] sm:max-w-none">
            {label}
          </span>
        </div>
        <div className="flex items-center gap-[12px] md:gap-[16px] shrink-0">
          <span className="font-mono bg-[var(--bg-base)] border border-[var(--border-default)] px-[10px] md:px-[12px] py-[2px] md:py-[4px] rounded-[6px] text-[11px] md:text-[12px] font-medium text-[var(--text-2)] whitespace-nowrap shrink-0">
            {count} Issues
          </span>
          {!forceOpen && count > 0 && (
            <ChevronDown
              size={18}
              className={`text-[var(--text-3)] transition-transform duration-300 shrink-0 ${
                isOpen ? 'rotate-180' : ''
              } md:w-[18px] md:h-[18px] w-[16px] h-[16px]`}
            />
          )}
        </div>
      </div>
          ...
          <div className="p-[16px] md:p-[24px] flex flex-col gap-[20px] md:gap-[24px] w-full min-w-0">
            {items &&
              items.map((item: any, idx: number) => (
                <div
                  key={idx}
                  className={`bg-[var(--bg-surface-1)] rounded-[10px] md:rounded-[12px] p-[20px] md:p-[24px] border-l-[4px] border border-[var(--border-subtle)] ${styles[type].bar} shadow-sm w-full min-w-0 shrink-0 ${
                    forceOpen ? 'break-inside-avoid' : ''
                  }`}
                >
                  <div className="flex justify-between items-start mb-[12px] md:mb-[16px] w-full min-w-0">
                    <h4 className="text-[var(--text-1)] font-medium text-[15px] md:text-[16px] break-words w-full">
                      {item.title}
                    </h4>
                  </div>
                  <p className="text-[var(--text-2)] text-[13px] md:text-[14px] leading-[1.6] md:leading-[1.7] mb-[20px] md:mb-[24px] max-w-full break-words">
                    {item.description}
                  </p>
                  ...
                  <div className="flex flex-col gap-[20px] md:gap-[24px] w-full min-w-0">
                        ...
                        <div className="bg-[#0A0D14] border border-[#FF3B30]/30 rounded-[12px] overflow-hidden shadow-2xl relative">
                          ...
                          {!forceOpen && (
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                navigator.clipboard.writeText(item.foundry_poc);
                                showAlert('Exploit copied to clipboard!');
                              }}
                    ...
                    {item.vulnerable_code && (
                      <div className={`flex flex-col w-full min-w-0 ${forceOpen ? 'break-inside-avoid' : ''}`}>
                        <span className="text-[10px] md:text-[11px] uppercase tracking-[0.08em] font-bold text-[var(--critical)] mb-[10px] md:mb-[12px] flex items-center gap-[8px] shrink-0">
                          <X size={12} className="md:w-[14px] md:h-[14px]" /> Vulnerable Logic
                        </span>
                    ...
                    {/* Remediated Code Segment */}
                    {item.remediated_code && (
                      <div className={`flex flex-col relative group w-full min-w-0 ${forceOpen ? 'break-inside-avoid' : ''}`}>
                        <span className="text-[10px] md:text-[11px] uppercase tracking-[0.08em] font-bold text-[var(--low)] mb-[10px] md:mb-[12px] flex items-center gap-[8px] shrink-0">
                          <CheckCircle size={12} className="md:w-[14px] md:h-[14px]" /> Remediated Code (Patch)
                        </span>
                        ...
                        {!forceOpen && (
                          <button
                            onClick={e => {
                              e.stopPropagation();
                              navigator.clipboard.writeText(item.remediated_code);
                              showAlert('Patch copied to clipboard!');
                            }}
